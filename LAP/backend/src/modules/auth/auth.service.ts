...
export class AuthService {
  ...
  async login(dto: LoginDto, response: Response) {
    ...
    const accessToken = await this.jwt.signAsync({ sub: user.id, email: user.email, roles, permissions });
    ...
    return { data: { accessToken, user: { id: user.id, name: user.name, email: user.email, roles, permissions } } };
  }
