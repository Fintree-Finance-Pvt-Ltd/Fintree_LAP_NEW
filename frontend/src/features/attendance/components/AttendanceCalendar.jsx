...
const WEEKDAY_NAMES = [
  { full: "Sunday", short: "Sun", isWeekend: true },
  { full: "Monday", short: "Mon", isWeekend: false },
  { full: "Tuesday", short: "Tue", isWeekend: false },
  { full: "Wednesday", short: "Wed", isWeekend: false },
  { full: "Thursday", short: "Thu", isWeekend: false },
  { full: "Friday", short: "Fri", isWeekend: false },
  { full: "Saturday", short: "Sat", isWeekend: false },
];
...
export default function AttendanceCalendar({
  ...
  const calendarDays = useMemo(() => {
    ...
    for (let dayNum = 1; dayNum <= daysInMonth; dayNum++) {
      ...
      // 1. Total work is 8.30 hours (510 minutes).
  ...
  return (
    <div className="space-y-5 animate-fadeIn">
      {/* Top Header Card: Month Navigation & Employee Selector */}
          ...
          <div className="flex flex-wrap items-center gap-3">
            <div className="flex items-center rounded-xl border border-slate-200 bg-slate-50 p-1 shadow-2xs">
              <button
                ...
                className="flex h-9 w-9 items-center justify-center rounded-lg text-slate-600 hover:bg-white hover:text-[#0f2942] hover:shadow-2xs transition active:scale-95 cursor-pointer"
                title="Previous Month"
              >
                <FiChevronLeft className="h-5 w-5" />
              </button>

              <div className="px-4 font-bold text-sm sm:text-base text-[#0f2942] min-w-[160px] text-center">
            ...
            <div className="flex items-center gap-1.5 rounded-xl border border-blue-200/80 bg-blue-50/80 px-3 py-2 text-xs font-semibold text-blue-800 shadow-2xs">
              <FiClock className="h-3.5 w-3.5 text-blue-600 shrink-0" />
              <span>Shift Target: <strong>{TARGET_HOURS_LABEL}</strong></span>
            </div>
          </div>
            ...
            {isAdminOrBM && Array.isArray(allUsers) && allUsers.length > 0 && (
              <div className="flex items-center gap-2">
                ...
                <select
                  ...
                  className="rounded-xl border border-slate-200 bg-white px-3 py-2 text-xs font-semibold text-slate-700 shadow-2xs focus:outline-none focus:ring-2 focus:ring-blue-500/20"
                >
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-6">
        ...
        {/* Sunday Off Card */}
        <div className="rounded-2xl border border-slate-200/90 bg-slate-100/70 p-3.5 shadow-2xs">
          <div className="flex items-center justify-between">
        ...
        <div className="rounded-2xl border border-blue-200/90 bg-blue-50/60 p-3.5 shadow-2xs">
          ...
          <div className="mt-2 text-2xl font-black text-blue-950 font-mono">
            {monthlyStats.totalHours} <span className="text-xs font-bold text-blue-700">hrs</span>
          </div>
        ...
        {/* Daily Average Card */}
        <div className="rounded-2xl border border-indigo-200/90 bg-indigo-50/60 p-3.5 shadow-2xs">
          <div className="flex items-center justify-between">
          ...
          <div className="mt-2 text-2xl font-black text-indigo-950 font-mono">
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 text-[11px]">
          ...
          <div className="flex flex-wrap items-center gap-3 sm:gap-5">
            ...
            <div className="flex items-center gap-1.5">
              <span className="h-3.5 w-3.5 rounded-md bg-emerald-500 border border-emerald-600" />
            ...
            {/* Amber */}
            <div className="flex items-center gap-1.5">
              <span className="h-3.5 w-3.5 rounded-md bg-amber-400 border border-amber-500" />
              <span className="font-semibold text-slate-700">
                Half Day (&lt; 8h 30m)
              </span>
            </div>
            ...
            {/* Red */}
            <div className="flex items-center gap-1.5">
              <span className="h-3.5 w-3.5 rounded-md bg-rose-500 border border-rose-600" />
              <span className="font-semibold text-slate-700">
                Absent (Working Day)
              </span>
            </div>
            ...
            {/* Gray */}
            <div className="flex items-center gap-1.5">
              <span className="h-3.5 w-3.5 rounded-md bg-slate-300 border border-slate-400" />
              <span className="font-semibold text-slate-700">
                Sunday (Off)
              </span>
            </div>
            ...
            {/* Sunday Worked */}
            <div className="flex items-center gap-1.5">
              <span className="h-3.5 w-3.5 rounded-md bg-emerald-100 border-2 border-dashed border-emerald-600" />
              <span className="font-semibold text-slate-700">
                Sunday Worked
              </span>
            </div>
      ...
      <div className="rounded-2xl border border-slate-200/80 bg-white p-3 sm:p-5 shadow-xs overflow-hidden">
        {/* Weekday Column Headers */}
        <div className="grid grid-cols-7 gap-1.5 sm:gap-2 text-center border-b border-slate-200/80 pb-3 mb-2">
          {WEEKDAY_NAMES.map((wd) => (
            <div
              key={wd.short}
              className={`py-1 text-xs font-extrabold uppercase tracking-wider ${
                wd.isWeekend
                  ? "text-rose-600 bg-rose-50/60 rounded-lg"
                  : "text-slate-600"
              }`}
            >
          ...
          {calendarDays.map((day, idx) => {
            if (!day.isCurrentMonth) {
              return (
                <div
                  key={`inactive-${idx}`}
                  className="min-h-[85px] sm:min-h-[115px] rounded-xl border border-dashed border-slate-100 bg-slate-50/30 p-2 text-slate-300 select-none opacity-40"
                >
            ...
            const isClickable = hasRecord || day.statusType === "ABSENT" || day.statusType === "SUNDAY_OFF";

            return (
              <div
                key={day.dateStr}
                onClick={() => isClickable && setSelectedDayData(day)}
                className={`group relative flex flex-col justify-between min-h-[85px] sm:min-h-[115px] rounded-xl border p-2 sm:p-2.5 transition-all duration-200 shadow-2xs ${
                  day.bgClasses
                } ${day.borderClasses} ${
              ...
              >
                ...
                {/* Day Content / Status Details */}
                <div className="my-1 sm:my-1.5 space-y-1">
                  {/* Status Badge */}
                  {day.badgeLabel && (
                    <div className="flex items-center justify-start">
                      <span
                        className={`inline-flex items-center gap-1 rounded-md px-1.5 py-0.5 text-[9px] sm:text-[10px] font-bold truncate max-w-full ${
                          day.badgeClasses
                        }`}
                        title={day.badgeLabel}
                      >
                        {day.statusType === "FULL_DAY" && "🟢"}
                        {day.statusType === "SHORT_DAY" && "🟡"}
                        {day.statusType === "ABSENT" && "🔴"}
                        {day.statusType === "SUNDAY_OFF" && "⚪"}
                        {day.statusType === "SUNDAY_WORKED_FULL" && "🟢"}
                        {day.statusType === "SUNDAY_WORKED_PARTIAL" && "🟡"}
                  ...
                  {hasRecord && (
                    <div className="hidden sm:block space-y-0.5 text-[10px] font-mono font-semibold">
                  ...
                  {/* Absent message */}
                  {day.statusType === "ABSENT" && (
                    <div className="hidden sm:block text-[10px] font-medium text-rose-700 leading-tight">
                      No punch-in recorded
                    </div>
                  )}
                </div>

                {/* Day Footer: Total Hours Duration & Distance */}
                <div className="flex items-center justify-between border-t border-black/5 pt-1 text-[10px]">
                  {hasRecord || day.statusType === "IN_PROGRESS" ? (
                    <>
                      <div
                        ...
                        title={`Logged: ${day.durationFormatted} / Standard: ${TARGET_HOURS_LABEL}`}
                      >
