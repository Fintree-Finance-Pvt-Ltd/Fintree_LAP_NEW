...
const EMPTY_GEO_DATA = {
  residence: { lat: "", lng: "", gpsAddress: "", accuracyMeters: "", capturedAt: "", status: "Pending" },
  business: { lat: "", lng: "", gpsAddress: "", accuracyMeters: "", capturedAt: "", status: "Pending" },
  property: { lat: "", lng: "", gpsAddress: "", accuracyMeters: "", capturedAt: "", distanceSpoke: "", mismatch: "No", status: "Pending" },
};

const LOCATION_TYPE_MAP = { residence: "RESIDENCE", business: "BUSINESS", property: "PROPERTY" };
const TAB_FROM_LOCATION_TYPE = { RESIDENCE: "residence", BUSINESS: "business", PROPERTY: "property" };

const GEO_TABS = [
  { key: "residence", label: "Residence Geo" },
  { key: "business", label: "Business Geo" },
  { key: "property", label: "Property Geo" },
];

const unwrapResponse = (response) => response?.data !== undefined ? response.data : (response ?? {});
...
const getLocationErrorMessage = (error) => {
  switch (error?.code) {
    case 1: return "Location permission denied. Please allow location access in your browser.";
    case 2: return "Your current location is unavailable.";
    case 3: return "Location request timed out. Please try again.";
    default: return "Unable to capture your current location.";
  }
};
...
export default function GeoVerification() {
  ...
  const [geoData, setGeoData] = useState(EMPTY_GEO_DATA);
  ...
  const geoPrerequisitesMet =
    ...
    (Boolean(apiWorkflowFlags.customerVisit) && Boolean(apiWorkflowFlags.businessVisit));

  const geoBlocked = Boolean(applicationId) && workflowData.isSuccess && !geoPrerequisitesMet;
  ...
  useEffect(() => {
    ...
    setGeoData((previous) => {
      ...
      locations.forEach((location) => {
        const tabKey = TAB_FROM_LOCATION_TYPE[location.locationType];
        if (!tabKey) return;
        ...
        updated[tabKey] = {
          ...
          accuracyMeters: location.accuracyMeters != null ? String(location.accuracyMeters) : "",
  ...
  const handleCaptureLiveLocation = () => {
    if (!Number.isInteger(numericApplicationId) || numericApplicationId <= 0) {
      setMessageType("error");
      setMessage("A valid application ID is required before capturing location.");
      return;
    }
    ...
    navigator.geolocation.getCurrentPosition(
      (position) => {
        ...
        const coordinateAddress = `Latitude ${latitude.toFixed(7)}, Longitude ${longitude.toFixed(7)}`;

        saveGeoLocationMutation.mutate({
          locationKey,
          payload: { locationType, latitude, longitude, accuracyMeters: accuracy, gpsAddress: coordinateAddress },
        });
      },
      (error) => {
  ...
  const hasSelectedCoordinates = selectedGeoData?.lat !== "" && selectedGeoData?.lng !== "" && Number.isFinite(Number(selectedGeoData?.lat)) && Number.isFinite(Number(selectedGeoData?.lng));

  const handleOpenGoogleMaps = () => {
    if (!hasSelectedCoordinates) return;
    window.open(`https://www.google.com/maps/search/?api=1&query=${selectedGeoData.lat},${selectedGeoData.lng}`, "_blank", "noopener,noreferrer");
  };

  const currentTabLabel = GEO_TABS.find((tab) => tab.key === activeTab)?.label || "Geo";
  const isCapturing = Boolean(capturingLocationType) || saveGeoLocationMutation.isPending;

  // Calculate global step states using sequential workflow logic
    ...
    <div className="min-h-screen space-y-6 bg-[#f8fafc] p-4 text-slate-800 antialiased md:p-8">
      ...
      <div className="relative overflow-hidden rounded-3xl bg-gradient-to-r from-[#1e3a8a] via-[#2563eb] to-[#3b82f6] p-8 text-white shadow-xl shadow-blue-900/10">
        ...
        <div className="relative z-10 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <h2 className="text-2xl font-bold tracking-tight sm:text-3xl">Geo Verification</h2>
            <p className="mt-1 text-xs font-medium text-blue-100/90">
              Application #{applicationId || "—"} • Real-time field parameter checks.
            </p>
          </div>
      ...
      {/* NEW REDESIGNED LIVE APPLICATION JOURNEY METRIC BOARD */}
        ...
        <div className="mb-6 flex items-center justify-between">
          <div className="space-y-0.5">
            ...
            <p className="text-sm font-extrabold text-slate-800">
              Pipeline Tracking ID: <span className="text-blue-600">#{applicationId || "Unselected"}</span>
            </p>
          </div>
          {workflowData.isFetching && (
            <span className="inline-flex items-center rounded-md bg-blue-50 px-2.5 py-1 text-xs font-medium text-blue-700 animate-pulse">
              Syncing milestones...
            </span>
          )}
        </div>

        {/* Horizontal Pipeline System */}
      ...
      <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
            ...
            {GEO_TABS.map((tab) => {
              ...
              return (
                ...
                >
                  ...
                  {isActive && <span className="absolute bottom-0 left-0 h-0.5 w-full bg-blue-600" />}
                </button>
              );
            })}
          </div>
        ...
        <div className="p-6 md:p-8">
          ...
          <div className="grid grid-cols-1 gap-5 md:grid-cols-2">
            ...
            <GeoField label="Accuracy" value={selectedGeoData.accuracyMeters ? `${selectedGeoData.accuracyMeters} meters` : ""} />
            <GeoField label="Captured At" value={selectedGeoData.capturedAt ? formatCapturedAt(selectedGeoData.capturedAt) : ""} />

            <div className="md:col-span-2">
              <GeoField label="GPS Location Address" value={selectedGeoData.gpsAddress} />
            </div>

            {activeTab === "property" && (
              <>
                <div className="flex flex-col gap-1.5">
                  <label className="text-[11px] font-bold uppercase tracking-wider text-slate-400">Distance from Spoke (KM)</label>
                  <input
                    type="number"
                    value={geoData.property.distanceSpoke}
                    onChange={(e) => setGeoData(prev => ({ ...prev, property: { ...prev.property, distanceSpoke: e.target.value } }))}
                    placeholder="Enter distance"
                    className="w-full rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-sm font-medium text-slate-700 outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-50 text-slate-800 transition-all"
                  />
                </div>
                <div className="flex flex-col gap-1.5">
                  <label className="text-[11px] font-bold uppercase tracking-wider text-slate-400">Distance Mismatch</label>
                  <select
                    value={geoData.property.mismatch}
                    onChange={(e) => setGeoData(prev => ({ ...prev, property: { ...prev.property, mismatch: e.target.value } }))}
      ...
      <div className="grid grid-cols-1 gap-6 xl:grid-cols-3">
        <div className="flex flex-col rounded-2xl border border-slate-100 bg-white p-6 shadow-sm xl:col-span-2">
          <div className="mb-4 flex items-center gap-2 border-b border-slate-100 pb-2">
            <h3 className="text-xs font-bold uppercase tracking-wider text-slate-400">Live Mapping Preview</h3>
          </div>
          <div className="relative flex min-h-[260px] flex-1 flex-col items-center justify-center rounded-xl bg-slate-50 border border-dashed border-slate-200 p-8 text-center">
            {hasSelectedCoordinates ? (
              <>
                <div className="h-3 w-3 rounded-full bg-rose-500 animate-ping absolute top-4 right-4" />
                <h4 className="max-w-md text-sm font-bold text-slate-800">{selectedGeoData.gpsAddress}</h4>
                <p className="mt-1 font-mono text-xs text-slate-400">{selectedGeoData.lat}, {selectedGeoData.lng}</p>
              </>
            ) : (
              <>
                <p className="text-sm font-bold text-slate-600">No Location Registered</p>
                <p className="mt-1 text-xs text-slate-400">Please capture data using the browser action handlers above.</p>
              </>
            )}
            <button
              type="button"
            ...
            >
              Open Live External Map <FaExternalLinkAlt size={10} />
            </button>
          </div>
        </div>

        {/* METRIC MATRIX CONTROL PANEL */}
        <div className="flex flex-col justify-between rounded-2xl border border-slate-100 bg-white p-6 shadow-sm">
          <div className="space-y-4">
            <div className="border-b border-slate-100 pb-2">
              <h3 className="text-xs font-bold uppercase tracking-wider text-slate-400">Geo Summary</h3>
            </div>
            <div className="space-y-3 pt-1 text-xs font-semibold text-slate-600">
              <div className="flex justify-between border-b border-slate-50 pb-2">
                <span>Target Node</span>
                <span className="font-bold text-slate-800">{currentTabLabel}</span>
              </div>
              <div className="flex justify-between border-b border-slate-50 pb-2">
                <span>Verification State</span>
                <span className={`font-bold ${selectedGeoData.status === "Verified" ? "text-emerald-600" : "text-amber-600"}`}>
                  {selectedGeoData.status}
                </span>
              </div>
            </div>
