...
const handleViewFieldVisitDocument = async (fieldVisitDocument) => {
  ...
  const metrics = [
    ...
    },
    {
      ...
      value: requiredDocumentTypes.length,
    ...
    {
      title: "KYC STATUS",
      value: completion === 100 ? "Ready" : "Pending",
      color: "from-amber-500/10 to-orange-500/5",
      textColor: "text-amber-600",
    },
  ];

  const formatDocumentName = (value) => {
    if (!value) return "Field Visit Document";
    return String(value)
      .toLowerCase()
      .split("_")
      .filter(Boolean)
  ...
  const upload = useMutation({
    mutationFn: ({ applicationId, documentType, documentName, file }) => {
      ...
      formData.append("documentType", String(documentType).trim().toUpperCase());
      formData.append("documentName", String(documentName || documentType).trim());
      formData.append("documentSource", "RM_PORTAL");
  ...
  const handleAddDocument = () => {
    ...
    if (!allowedTypes.includes(newDocumentFile.type)) {
      setAddDocumentError("Only PDF, JPG and PNG files are allowed.");
      return;
    }

    const maximumFileSize = 15 * 1024 * 1024;
    if (newDocumentFile.size > maximumFileSize) {
      setAddDocumentError("File size must not exceed 15 MB.");
      return;
    }
      ...
      <div className="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-4">
        {metrics.map((card) => (
          <div key={card.title} className={`rounded-2xl border border-white bg-white bg-gradient-to-br ${card.color} p-6 shadow-sm`}>
            ...
            <div className={`mt-2 text-3xl font-extrabold tracking-tight ${card.textColor}`}>
              {documentsQuery.isLoading ? "--" : card.value}
            </div>
          </div>
        ))}
      </div>
      ...
      <div className="overflow-hidden rounded-2xl border border-slate-100 bg-white shadow-sm">
        <div className="flex flex-col gap-3 border-b border-slate-100 p-6 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <p className="mt-1 text-[11px] font-medium text-slate-400">Upload and manage applicant KYC documents</p>
          </div>
          <button
            type="button"
        ...
        <div className="overflow-x-auto">
          <table className="w-full border-collapse text-left">
            ...
            <tbody className="divide-y divide-slate-100 text-xs font-medium text-slate-700">
                ...
                .map((type) => {
                  ...
                  return (
                    <tr key={type} className="transition-colors hover:bg-slate-50/40">
                      ...
                      <td className="p-4">
                        <span className="text-[10px] font-semibold text-slate-400">System Checklist</span>
                      </td>
              ...
              {/* 2. Custom Uploaded / Additional documents */}
                  ...
                  <tr key={`field-visit-${document.id}`} className="transition-colors hover:bg-slate-50/40">
                    <td className="p-4 pl-6">
                      ...
                      <div className="mt-0.5 text-[10px] font-medium text-slate-400">
                        {document.visitType ? formatDocumentName(document.visitType) : "Field Visit Photo"}
                      </div>
                    </td>
                    <td className="p-4">
                      <p className="max-w-[220px] truncate font-semibold text-slate-500" title={document.fileName || ""}>
      ...
      <div className="grid grid-cols-1 gap-6 xl:grid-cols-5">
        <div className="space-y-4 rounded-2xl border border-slate-100 bg-white p-6 shadow-sm xl:col-span-2">
          <h3 className="flex items-center gap-2 text-sm font-extrabold tracking-wide text-[#0f2942]">
            <FaCheckCircle className="text-emerald-500" /> KYC results
          </h3>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-950/50 p-4 backdrop-blur-sm">
          <div className="w-full max-w-md overflow-hidden rounded-2xl bg-white shadow-2xl">
            <div className="flex items-center justify-between border-b border-slate-100 px-6 py-4">
              <div>
                <h3 className="text-sm font-extrabold text-[#0f2942]">Add KYC Document</h3>
                <p className="mt-1 text-[11px] font-medium text-slate-400">Upload an additional customer document</p>
              </div>
              <button
                type="button"
            ...
            <div className="space-y-5 p-6">
              <div>
                <label className="mb-1.5 block text-[11px] font-bold uppercase tracking-wide text-slate-500">
                  Document Name <span className="ml-1 text-rose-500">*</span>
                </label>
                <input
                  type="text"
              ...
              <div>
                <label className="mb-1.5 block text-[11px] font-bold uppercase tracking-wide text-slate-500">
                  Select File <span className="ml-1 text-rose-500">*</span>
                </label>
                <label
                ...
                >
                  ...
                  {newDocumentFile ? (
                    <>
                      <p className="truncate text-xs font-bold text-blue-700">{newDocumentFile.name}</p>
                    ...
                    <>
                      <p className="text-xs font-bold text-slate-700">Click to select a file</p>
                      <p className="mt-1 text-[10px] font-medium text-slate-400">PDF, JPG or PNG, maximum 15 MB</p>
                    </>
                  )}
                </label>
              </div>
            ...
            <div className="flex justify-end gap-3 border-t border-slate-100 bg-slate-50/50 px-6 py-4">
              ...
              <button
                type="button"
                disabled={upload.isPending || !newDocumentName.trim() || !newDocumentFile}
              ...
              >
                {upload.isPending && uploadingType === "ADDITIONAL_DOCUMENT" ? "Uploading..." : "Upload Document"}
              </button>
            </div>
          </div>
        </div>
...
function Result({ label, value }) {
  ...
  return (
    <div className="flex items-center justify-between py-2.5 text-xs">
      ...
      <span className={`rounded px-2 py-0.5 text-[10px] font-bold ${ready ? "border border-emerald-100 bg-emerald-50 text-emerald-600" : "border border-rose-100 bg-rose-50 text-rose-600"}`}>
        {value}
      </span>
    </div>
  );
