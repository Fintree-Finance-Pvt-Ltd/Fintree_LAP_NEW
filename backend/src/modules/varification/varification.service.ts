...
async verifyGst(gstNumber: string, applicationId: number) {
  ...
  try {
    ...
    try {
      ...
      this.logger.log(
        `GST sandbox response: status=${sandboxResponse.status}, data=${JSON.stringify(
          sandboxResponse?.data ?? null,
        )}`,
      );
      ...
      sandboxResult = {
        ...
        error: sandboxError?.response?.data ?? sandboxError?.message ?? null,
      };
...
private async callZoopGstVerification(gstNumber: string) {
  ...
  this.logger.log(
    `GST Zoop fallback response: status=${response.status}, data=${JSON.stringify(
      zoopData,
    )}`,
  );
...
if (!email) {
  ...
  try {
    ...
    this.logger.log(
      `Aadhaar init response: status=${response.status}, data=${JSON.stringify(
        aadhaarApiResponse,
      )}`,
    );
    ...
    const model = aadhaarApiResponse?.model || aadhaarApiResponse?.data || {};
...
try {
  await this.sendAadhaarKycEmail({
    to: email,
    customerName,
    applicationNumber: application.applicationNumber || `APP-${applicationId}`,
    kycUrl,
    uniqueId,
  ...
  this.logger.error(
    `Aadhaar KYC email failed for ${email}: ${mailError?.message || mailError}`,
  );

  throw new HttpException(
    `Aadhaar KYC link generated but email sending failed. ${mailError?.message || ''}`,
    400,
  );
}
...
async updateAadhaarKycStatus(body: any) {
  const transactionId =
    body?.unifiedTransactionId ||
    body?.transactionId ||
    body?.model?.unifiedTransactionId ||
    body?.data?.unifiedTransactionId ||
    body?.uniqueId;

  if (!transactionId) {
    throw new HttpException('Aadhaar transaction id is required', 400);
  }

  const aadhaarWebhookResponse = body;

  await this.dataSource.transaction(async (manager) => {
    const kycStatus = await manager.findOne(KycVerificationStatus, {
      where: {
    ...
    if (!kycStatus) {
      throw new NotFoundException(
        `Aadhaar transaction ${transactionId} was not found.`,
      );
    }

    const applicationId = Number(kycStatus.applicationId);
    ...
    kycStatus.aadhaarStatus = KycVerificationStatusValue.VERIFIED;
    kycStatus.aadhaarName = aadhaarName;
    kycStatus.aadhaarMaskedNumber = maskedAadhaar;
    kycStatus.aadhaarDob = dob;
    kycStatus.aadhaarAddress =
      typeof address === 'string' ? address : JSON.stringify(address);
    kycStatus.aadhaarWebhookResponse =
      JSON.stringify(aadhaarWebhookResponse);

    if (profile) {
      profile.aadhaarVerified = true;

      if (maskedAadhaar) {
        profile.aadhaarNumber = String(maskedAadhaar)
          .replace(/\D/g, '')
          .slice(-12);
      }
  ...
  return {
    success: true,
    message: 'Aadhaar KYC status updated successfully',
    data: {
      transactionId,
...
private async sendAadhaarKycEmail(params: {
  ...
  const { to, customerName, applicationNumber, kycUrl, uniqueId, unifiedTransactionId } =
    params;
    ...
    <div style="font-family: Arial, sans-serif; background:#f8fafc; padding:24px;">
      <div style="max-width:620px; margin:auto; background:#ffffff; border:1px solid #e2e8f0; border-radius:14px; overflow:hidden;">
        ...
        <div style="padding:24px;">
          <p style="font-size:14px; color:#334155;">Dear ${customerName || 'Customer'},</p>
          ...
          <div style="text-align:center; margin:26px 0;">
            <a href="${kycUrl}"
               style="display:inline-block; background:#2563eb; color:#ffffff; text-decoration:none; padding:13px 22px; border-radius:10px; font-size:14px; font-weight:bold;">
  ...
  await transporter.sendMail({
    from: smtpFrom,
    to,
    subject: `Complete Aadhaar KYC - ${applicationNumber || 'Fintree LAP'}`,
    html,
  });
}
